...
- **エグジット戦略**:
    - **Long**: シグナルベースまたはATRトレーリングストップ（✅ 実装済み）。
    - **Short**: **ATRベース動的損切り(1.5x) / 利確(2.5x)**（✅ 実装済み）。
- **統計的検証**: ウォークフォワード検証、モンテカルロシミュレーション（✅ 実装済み）。

### 3.8 売買ロジック詳細 (Current Logic)
**共通エントリー条件**:
- **シグナル**: 総合スコア 6.5以上 (BUY)
- **クールダウン**: 損切り後5日間 / 利確後3日間は再エントリー禁止

**戦略別エグジット条件**:
| 戦略 | プライマリ (優先) | セカンダリ (保険) | 備考 |
| :--- | :--- | :--- | :--- |
| **Long (長期)** | **ATR Trailing Stop**<br>高値から `3.0 x ATR` 下落で決済 | **Signal SELL**<br>総合スコア 3.5以下 | トレンド追随型。<br>調整局面での一時的なスコア低下（WATCH）では売らず、トレンド崩壊（ATR）かファンダ悪化（Signal）で逃げる。 |
| **Short (短期)** | **ATR Bracket**<br>損切り: `-1.5 x ATR`<br>利確: `+2.5 x ATR` | **Signal SELL**<br>または固定% (-3%/+5%) | ボラティリティブレイクアウト型。<br>日次で細かく利益を積み上げる。ATR取得失敗時は固定%で安全装置発動。 |
