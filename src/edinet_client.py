...
# HIGH-004: 循環インポート回避のため、data_fetcher のインポートは関数内で行う
# call_gemini のみが必要なため、_analyze_yuho_with_gemini() 内でインポートする
...
def _download_edinet_code_list() -> None:
    """
    EDINETコードリスト（ZIP→CSV）をダウンロードしてキャッシュ。
    """
    url = "https://disclosure2dl.edinet-fsa.go.jp/searchdocument/codelist/Edinetcode.zip"

    session = requests.Session()
    session.max_redirects = 30  # リダイレクトループ防止
...
def _load_edinet_code_map() -> dict:
    ...
    try:
        ...
        # EDINET v2の静的ZIPのCSVは1行目にメタデータ（ダウンロード実行日...）があるためスキップ
...
{clean_text}
    ...
    # HIGH-004: 循環インポート回避のため関数内でインポート
    from .data_fetcher import call_gemini
    result, model_name = call_gemini(prompt, parse_json=True, model="flash") # 高速化のためFlash推奨
...
def extract_yuho_data(ticker: str, skip_ai: bool = False) -> dict:
    ...
    非日本株やエラー時も生テキストだけは返すようフォールバック改善。
    """
    if not is_japanese_stock(ticker):
    ...
    # キャッシュがなければフルスキャン
    if not doc_info:
        # API キーチェックをここに移動（キャッシュはキー不要のため）
        if not _get_edinet_key():
