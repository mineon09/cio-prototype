...
| 原因 | 対処 |
|---|---|
| yfinance レート制限 (429 / Too Many Requests) | **自動フォールバック**（Layer 2: ステールキャッシュ 72h → Layer 3: Finnhub） |
...
Layer 1: yfinance（24h キャッシュ）
   ↓ 429 / Rate Limited
Layer 2: ステールキャッシュ再利用（72h まで許容）  → [RATE_LIMIT] ログ
   ↓ キャッシュなし or 72h 超
Layer 3: Finnhub API（FINNHUB_API_KEY が必要）     → [FINNHUB_FALLBACK] ログ
```
