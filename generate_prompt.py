...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
※ SEC EDGAR データの取得がスキップまたは失敗したため、あなたの知識から {ticker} の直近10-K/10-Qに基づき以下を分析してください：
...
_PARALLEL_FETCH_TIMEOUT = 90  # 各並列タスクの個別タイムアウト（秒）
...
def build_full_prompt(ticker: str, include_qualitative: bool = True):
    ...
    # --- 並列タスク定義（fetch_stock_data 完了後に独立して実行可能）---

    def _fetch_news():
        try:
            from src.news_fetcher import fetch_all_news
            return fetch_all_news(ticker, company_name=company_name, include_google=True)
        except Exception as e:
    ...
    def _fetch_analyst():
        try:
            ...
            return fetch_all_analyst_data(ticker)
        except Exception as e:
    ...
    def _fetch_edinetdb():
        try:
            ...
            return get_full_company_data(ticker)
        except Exception as e:
            print(f"  ⚠️ EDINET DB取得エラー：{e}")
            return None

    def _fetch_edinet_raw():
        """EDINET 有報 PDF からの生テキスト取得（edinetdb 未設定時のフォールバック）"""
        try:
            ...
            if raw_text:
                summary = _extract_yuho_sections(raw_text)
    ...
    def _fetch_jquants():
        ...
        try:
            ...
            if not hist.empty:
                ...
                print(f"  ✓ J-Quants フォールバック: yfinance から {len(rows)} 件の OHLC を取得")
    ...
    def _fetch_web_news():
        try:
            ...
            query = f"{company_name} 株価 業績 ニュース"
    ...
    web_news_data = None
    industry_data = None  # 廃止（Groq呼び出し排除）→ Claudeが直接分析

    task_fns: dict = {}
    if include_qualitative:
        task_fns["news"] = _fetch_news
        task_fns["analyst"] = _fetch_analyst
    if is_japanese:
        task_fns["edinetdb"] = _fetch_edinetdb
        task_fns["edinet_raw"] = _fetch_edinet_raw  # edinetdb 失敗時のフォールバック用に並列で先取り
    ...
    try:
        futures = {key: executor.submit(fn) for key, fn in task_fns.items()}
        for key, future in futures.items():
            try:
                result = future.result(timeout=_PARALLEL_FETCH_TIMEOUT)
                if key == "news":
                    news_data = result
                elif key == "analyst":
                    analyst_data = result
                    ...
                    web_news_data = result
            except concurrent.futures.TimeoutError:
                print(f"  ⏰ {key} タイムアウト（{_PARALLEL_FETCH_TIMEOUT}秒）—スキップ")
            except Exception as e:
